...
- **Clean-room only.** The reference library this targets parity with is GPL-3.0: never read, fetch, or paste its source (minified or not) into this project or into Claude Code sessions. Parity is defined by DESIGN.md's behavior specs and public docs, not their implementation.
- **Never name a competitor.** Not in code, comments, commit messages, docs, or this file — refer to "the reference library"/"the incumbent" instead, consistently. This applies even in project files that are checked into git (and therefore public) — the ban isn't about legal exposure (naming it and describing parity goals is not itself a licensing problem), it's a standing preference: don't mention competitors, anywhere, ever.
...
- **Single-file distribution is the primary artifact.** `npm run build` must always emit `dist/shoji.js` + `dist/shoji.css` (and `.min` variants): one JS file and one CSS file containing core **and all official plugins**, self-registering, zero setup — drop two `<script>`/`<link>` tags in and everything works. Tree-shakable per-plugin ESM entries are also emitted for bundler users, but the single-file pair is what releases are cut from and what the demo site loads; it must never break or lag behind. **Every file under `dist/` for a given release must be from the same build, no exceptions** — one `npm run build` run produces all of them together, and npm publish ships that whole tree as one atomic tarball, so the published package itself is never inconsistent. The one place this can _look_ violated after a release: jsDelivr caches each `dist/` file path independently (its own per-file edge TTL), so a version-less CDN URL (`cdn.jsdelivr.net/npm/@michaelyagi/shoji/dist/...`) can transiently resolve two different files (e.g. `shoji.js` vs `shoji.min.js`) to two different versions right after a release, until each file's own cache entry expires — confirmed directly (2026-08-28: `shoji.js` had the beta.10+ feature, `shoji.min.js` didn't, both fetched moments apart from the version-less URL, while `npm view @michaelyagi/shoji dist-tags` already showed `latest` correctly on beta.11). Don't mistake that for a real dist/ build or publish bug — verify via the npm registry (`npm view`) or a pinned-version CDN URL, not the version-less one, before concluding otherwise.
